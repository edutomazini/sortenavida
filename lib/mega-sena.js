...
const downloadResultadosMegaSena = (folder) => {
  const url = 'http://www1.caixa.gov.br/loterias/_arquivos/loterias/D_megase.zip';
  return util.downloadResultados(folder, url, './', 'D_MEGA.HTM');
}


const htmlToJson = async (htmlFile) => {
    ...
    fs.readFile(htmlFile, 'latin1', (err, html) => {
      ...
      trs.each(function (index, element) {
        ...
        if (tds && tds.length > 0) {
          ...
          sorteio.Estimativa_Premio = util.parseToFloat(getText(tds[19]));
...
module.exports = { htmlToJson, downloadResultadosMegaSena}